package com.display.managers
{
	...
	public class CreateItemManager
	{
		...
		public static function clearGarbage():void{
			for each(var arr:Array in key){
				clearKey(arr);
			}
		}
		
		protected static function clearKey(key:Array):void{
			var i:int = 0
			var arr:Array = []
			var d:Dictionary = new Dictionary()
			for each(var index:int in key.dict){
				arr.push(key[index])
				d[key[index]] = i
//				key.dict[item] = key.length
//				key.push(item);
				i++
			}
			arr.dict = d;
			arr.emptys = [arr.length]
			key = arr;
		}
