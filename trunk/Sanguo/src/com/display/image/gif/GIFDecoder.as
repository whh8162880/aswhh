...
package com.display.image.gif
{
	...
	
	{
		...
		private var width:int; // full image width
		private var height:int; // full image height
		private var gctFlag:Boolean; // global color table used
		private var gctSize:int; // size of global color table
		private var loopCount:int = 1; // iterations; 0 = repeat forever

		private var gct:Array; // global color table
		private var lct:Array;// local color table
		private var act:Array; // active color table

		private var bgIndex:int; // background color index
		private var bgColor:int // background color
		private var lastBgColor:int; // previous bg color
		private var pixelAspect:int; // pixel aspect ratio

		private var lctFlag:Boolean // local color table flag
		private var interlace:Boolean; // interlace flag
		private var lctSize:int; // local color table size

		private var ix:int;
		private var iy:int;
		private var iw:int;
		private var ih:int // current image rectangle
		private var lastRect:Rectangle // last image rect
		private var image:BitmapData; // current frame
		private var bitmap:BitmapData;
		private var lastImage:BitmapData // previous frame

		private var block:ByteArray; // current data block
		...
		private var transIndex:int; // transparent color index
		...
		private var frames:Array // frames read from current file
		 ...
		 private function getPixels( bitmap:BitmapData ):Array
		{
			...
			for (var th:int = 0; th < lngHeight; th++)
			{
				...
				{
					var color:Number = bitmap.getPixel (th, tw);
		...
		private function setPixels( pixels:Array ):void
		{
			...
			for (var th:int = 0; th < lngHeight; th++)
			{
				for (var tw:int = 0; tw < lngWidth; tw++)
				{
					var color:Number = pixels[int(count++)];
...
class GifFrame 
{
	...
	public function GifFrame( im:BitmapData, del:int ) 
	{
