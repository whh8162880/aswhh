...
//			DebugTrace(this.name +" set nextScene:" + name);
         ...
         *
         */
        public function regPanel(panel:Object, x:Number = NaN, y:Number = NaN, halign:String = null, valign:String = null):void
        {
            var p:Class
            if (!(panel is Class))
            {
                var str:String = getQualifiedClassName(panel);
                try
                {
                    p = getDefinitionByName(str) as Class;
                }
                catch (e:Error)
                {
                    p = UnKnow;
            ...
            if (panel is DisplayObject)
            {
                var vo:PanelDefine = getPanel(DisplayObject(panel));
                if (vo)
                {
            ...
            vo = new PanelDefine(p, x, y, halign, valign, panel as DisplayObject);
            _panels.push(vo);
        }
...
class PanelDefine
{
    public function PanelDefine(panel:Class, x:Number = 0, y:Number = 0, halign:String = null, valign:String = null, displayObject:DisplayObject = null)
    {
        this.panel = panel
        this.x = x;
        this.y = y;
